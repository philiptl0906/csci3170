...
public class Customer{
  ...
  private static Boolean listingAllCustomer(String a){
    try{
        ...
        if(aCUSTLIST.contains(a)) return true;
        return false;
    }catch(Exception err) {
    System.err.println(err);
    return false;
  ...
  private static void bookSearchISBN() { // Search by ISBN - Complete, TESTED. debug: add for user input
    ...
    try {
        ...
        while(!listingAllISBN(isbncharacter)){
            ...
            if(countex!=0 && !listingAllISBN(isbncharacter)) System.out.println("Invalid input");
            countex++;
        }
  ...
  private static void orderQuery(){ // Done and tested
      try{
        ...
        cid = sc.nextLine();
        if(counterx!=0 && !listingAllCustomer(cid)) System.out.println("Invalid input, No such customer_id");
        counterx++;
        }
  ...
  private static void orderCreation() {
    try { 
        ...
        while(!listingAllCustomer(cid)){
            cid = sc.nextLine();
            if(!listingAllCustomer(cid) && counterID!=0) System.out.println("Invalid input, there are no such Customer_id");
            counterID++;
        }
            ...
            if(isbnList.contains(isbn)){
                ...
                if (copies >= qty){
                    ...
                    String emptyOrders = String.format(
                        "insert into orders " +
                        "values(\"%s\",\"%s\",\"%s\",%d,\"%s\") ",order_id, SystemInter.getSystemDate(),"N",0,cid
                    );
...
private static void orderAltering(){ // Done checked and tested 
    try{
        ...
        while(!listingAllOrder(oid)){
            oid = sc.nextLine();
            if(numberx!=0 && !listingAllOrder(oid)) System.out.println("There is no such order id. Please re-input");
            numberx++;
        }
        //Add copies to order 
