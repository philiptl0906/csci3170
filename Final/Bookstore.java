...
public class Bookstore {
    ...
    private static void Npop(Scanner in) {
        try {
            ...
            String mysql1 = String.format("Select * " + "from ordering " + "order by ISBN");
            String isbnA = "";
            String isbnB = "";
            ResultSet rs1 = Philip.executeQuery(mysql1);
            if (!rs1.isBeforeFirst())
                System.out.println("There are no record in table book");
            else {
                while (rs1.next()) {
                    isbnA = rs1.getString("ISBN");
                    if (loopCounter == 0)
                        bookCounter++;
                    else if (isbnA.equals(isbnB))
                        continue;
                    else {
                        bookCounter++;
                    }
            ...
            String mysql2 = String.format("select * " + "from ordering " + "order by ISBN");
            ...
            if (!rs2.isBeforeFirst())
                System.out.println("There are no record in the table book");
            else {
                while (rs2.next()) {
                ...
                for (int a = 0; a < countFilter; a++) {
                    // query for title
                    String sqlquery = String.format("select * " + "from book " + "where ISBN = \"%s\" ",
                            isbnArrFilter[a]);

                    ResultSet sqlquery1 = Philip.executeQuery(sqlquery);
                    sqlquery1.next();
                    if (qtyArrFilter[a] != 0)
                        System.out.println(isbnArrFilter[a] + "      " + sqlquery1.getString("title") + "      "
                                + qtyArrFilter[a]);
    ...
    public static void run() {
        ...
        run: while (true) {
            ...
            try {
                String newIn = in.next();
                input = Integer.parseInt(newIn);
                System.out.println("");
            }
